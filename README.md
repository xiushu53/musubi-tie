...
- **`/visualize-map` (アクセシビリティ可視化ページ)**
  - **役割:** 「最近傍施設までの距離」や「施設の勢力圏（ボロノイ図）」といった、地域全体のアクセシビリティに関するデータを地図上で視覚的に表示します。利用者は、どのエリアで福祉サービスが手厚いか、または手薄であるかを直感的に把握できます。
...
3,4のコマンドは`public/data`ディレクトリにJSONファイルを生成します。この処理は、初回起動時および施設データ(`public/*/facilities.json`)に更新があった場合に必要です。
...
├── scripts/             # Geohashデータ生成やDBへのデータ移行などのNode.jsスクリプト
└── src/
