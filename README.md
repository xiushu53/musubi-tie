...
- **発信地メッシュデータのエクスポート**: 分析に使用した問い合わせ発信地（KDE）のメッシュデータをGeoJSON形式でエクスポートできます。これにより、QGISなどのGISソフトウェアでさらに高度な空間分析を行うことが可能です。
...
- **デモモード**: 環境変数 `RESEND_API_KEY` を設定しない、または空にすることで、メールを実際に送信せずに問い合わせ機能のフロー全体をテストできるデモモードが利用可能です。デモモード時も、問い合わせ履歴はデータベースに保存されます。
...
- **`/` (トップページ)**
  - **役割:** アプリケーションの概要、主要機能（施設検索、アクセシビリティ可視化）、技術スタック、使用データを紹介するランディングページ。各機能ページへの導線を提供します。
...
- `RESEND_API_KEY`: ResendのAPIキー。**この値を空（`""`）に設定すると、メールを実際に送信しない「デモモード」でアプリケーションが動作します。**
...
└── src/
    ...
    │   │   ├── analytics/ # 分析データ用API
    │   │   │   ├── export/
    │   │   │   │   └── mesh/        # 問い合わせ発信地メッシュ（KDE）のGeoJSONエクスポートAPI
